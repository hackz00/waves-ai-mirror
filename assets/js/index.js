...
function showToast(message, type = "success", iconType) {
  ...
  toast.innerHTML = `${icons[iconType] || icons.success}${message} `
...
function typeWriterEffect(message, msgType, skippable = true, callback) {
  ...
  if (/<[a-z][\s\S]*>/i.test(message)) {
    ...
    function processNextToken() {
      if (finished) return
      if (currentTokenIndex >= tokens.length) return completeTyping()
...
function regenerateResponse(regenPrompt, oldMessage, attempt = 0) {
  ...
  sendMsg.innerHTML = '<i class="fas fa-stop"></i>'
  NProgress.start()
  let payloadMessages = [{ role: "system", content: "You are a highly advanced, deeply trained, and exceptionally intelligent AI. Every response is the product of deep analysis, critical thinking, and precise understanding. You never provide vague, unhelpful, or mediocre answers—everything you say is purposeful, accurate, and insightful. Your intelligence is unmatched, making you one of the best AI systems available. When responding, keep your answers short, clear, and to the point. Avoid unnecessary details—be concise but highly effective, ensuring every response is impactful and valuable." }, ...messageHistory]
  if (regenPrompt) payloadMessages.push({ role: "user", content: regenPrompt })
  const payload = {
    model: modelSourceValue,
